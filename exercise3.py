...
class Rectangle:
    ...
    def contains(self, point: Point2D, tolerance: float = 0.0) -> bool:
        ...
        #         and reuse that here.
        return self.is_in_interval(point, tolerance)
    ...
    def is_in_interval(self, point, tolerance) -> bool: # Task A
        ll_px = point.x - self._lower_left.x
        ll_py = point.y - self._lower_left.y
        if ll_px >= -tolerance and ll_px <= self._dx + tolerance and ll_py >= -tolerance and ll_py <= self._dy + tolerance:
            return True
        else:
            return False
